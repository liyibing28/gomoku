...
io.on('connection', function (socket) {
    ...
    socket.on('disconnect', () => {
        ...
        for (let key in desks) {
            ...
            if (desks[key].White === _socketId || desks[key].Black === _socketId) {
                ...
                socketClients[desks[key].White === _socketId ? desks[key].Black : desks[key].White].socket.emit('break');
                delDesk(key);
            }
        }
        ...
        desks.forEach(function (item,index) {
            ...
            if (item.White === _socketId || item.Black === _socketId) {
                console.log('fasdefefcec检测到断开');
                socketClients[item.White === _socketId ? item.Black : item.White].socket.emit('break');
                delDesk(index);
            }
        });
        // for (let item of desks) {
    ...
    socket.on('clickPiece', function (data) {
        ...
        if (a) {
            ...
            socket1.emit('serverClickPiece', { x: data.x, y: data.y });
        }
    });
